...
WSJ query and aggregate metrics are recalculated from the saved runs at the reranked top-100 cutoff, with nDCG@10 leading and fixed first-stage Recall@100 shown as a ceiling. Its source context gives the TREC-1 corpus size and article count. WSJ article names may appear, but full article text never enters the browser; the call view shows only a short excerpt and the recorded source-token count. The MS MARCO view replays all supplied candidates, comparing the saved monoBERT and original-text JEV rankings. Its Recall@100 changes with rank, while Recall@1000 shows the fixed candidate ceiling. Expanded rows open directly to the recorded JEV request and answer, with the passage text in the request shape. The supplied candidate file order is not a lexical ranking; reranking time and estimated API cost come from the completed run.
