...
ln -s ../.env .env.local # once, if using the repository root .env
npm run db:seed
npm run dev
```

Set `DATABASE_URL` (direct Neon connection) in the ignored repository root `.env` before seeding. Set `DATABASE_URL_POOLED` there for the Next.js server, then make it visible from `simulator/` with an ignored `.env.local` symlink (`ln -s ../.env .env.local`) or configure it in the server environment. Open `http://localhost:3000`. The MS MARCO preparation uses the tokenizer environment and cached `castorini/monobert-large-msmarco` revision described in [the reranking protocol](../reranking/jev-comparison.md), plus the three local downloads described in [the passage protocol](../reranking/msmarco.md). The preparation files are needed only for importing the saved dataset into Neon; the web server reads Neon directly.
...
The import preparation writes ignored files to `../.cache/reranking-simulator/`; `SIMULATOR_CONTENT_DIR` can point the importer to another directory. The schema in `db/schema.sql` stores queries, candidates, rankings, recorded calls, metrics and display content. WSJ rows contain article names only: a database constraint rejects article text. The WSJ document API returns an article name and `[REDACTED_TOKENS N source tokens sent]` for each call, with no article body or excerpt. The count comes from the saved document token length or passage window span; recorded API input tokens, which also include the query and question, are shown separately. MS MARCO passage text is stored for its JEV call view. The complete WSJ text never enters Neon, page props, API JSON or the browser. Keep the import preparation files and database access under the applicable collection terms.

## Evidence and scope

- `data/evidence.json` and `data/msmarco-evidence.json` are text-free, reproducible import artifacts containing IDs, ranks, judgments, recorded scores, usage, timing and metrics. The scripts rebuild them from completed repository artifacts and validate candidate membership and scoring coverage. The web server reads the corresponding Neon tables at runtime.
- Each dropdown shows an alphabetized sample of 20 queries spaced across the dataset's query IDs, including its default query. The WSJ and MS MARCO aggregate metrics still use all 50 and 43 queries respectively.
- The local text preparation checks JEV payload hashes before import. The WSJ call inspector shows a redacted request shape and recorded score/model/usage fields; MS MARCO displays the saved passage input. Both are replays, not fresh model responses.
...
- The WSJ corpus is the TREC-1 subset of TREC disks 1 and 2: 173,252 indexed articles across the 1987–1992 volumes, about 0.5 GB of source text. The simulator links to NIST collection statistics; no article body is stored in its database or bundled with the app.
