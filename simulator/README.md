...
- Each saved WSJ BM25 search has 1,000 results. JEV reranks positions 1–100 only; positions 101–1,000 keep their BM25 order, and the UI shows the top ten rows. WSJ query and aggregate metrics use `trec_eval -c -M100` on the saved run and qrels. Top-100 MAP is 0.1729 for BM25 and 0.2263 for complete-document JEV. The original full 1,000-result MAP values remain 0.2521 and 0.3055 in the experiment reports; these are different evaluation scopes.
...
- The MS MARCO v1 corpus contains 8,841,823 passages drawn from millions of web pages. This TREC DL 2019 replay reranks only 41,042 supplied query–passage pairs across 43 judged queries. monoBERT provides a comparable reranker on those same candidates; the page starts from its saved order.
