...
export type DocumentPayload = {
  ...
  title: string
  text: string | null
  calls: (ScoreRecord & { payload: string | null })[]
}
    ...
    <main className="site-shell">
      ...
      <div className="page-wrap">
        ...
        <section className="ranking-section" aria-labelledby="rank-title">
          ...
          {!payload.contentAvailable && <div className="content-alert"><DocumentTextIcon className="size-5" /><span>Article text is not installed on this server. Rankings and measured scores still replay; prepare a licensed local WSJ collection to inspect content and full call payloads.</span></div>}
            ...
            <AnimatePresence initial={false} mode="popLayout">
              {visible.map((docid, index) => {
                ...
                return <motion.li key={docid} layout="position" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }} transition={{ layout: { type: 'spring', stiffness: 340, damping: 34 }, opacity: { duration: 0.2 } }} className={`result-row ${isOpen ? 'result-open' : ''}`}>
                  ...
                  <AnimatePresence initial={false}>{isOpen && <motion.div className="result-details" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: reduced ? 0 : 0.28, ease: [0.16, 1, 0.3, 1] }}><div className="details-inner"><div className="details-tabs" role="tablist" aria-label={`Inspect ${docid}`}><button type="button" role="tab" aria-selected={activeTab === 'article'} onClick={() => setActiveTab('article')}><DocumentTextIcon className="size-4" /> Article</button><button type="button" role="tab" aria-selected={activeTab === 'call'} onClick={() => setActiveTab('call')}><CommandLineIcon className="size-4" /> JEV call + result</button></div>{documentError ? <div className="inline-error">{documentError}</div> : !details ? <div className="details-loading">Loading the saved result…</div> : activeTab === 'article' ? <div className="article-pane"><div className="pane-head"><strong>Article {docid}</strong><span>{details.text ? `${details.text.length.toLocaleString()} characters` : 'Text unavailable'}</span></div>{details.text ? <p>{details.text}</p> : <p>Article text is not installed on this server. The scored document ID, judgment and ranking remain available.</p>}</div> : <CallPane details={details} task={task} query={payload.text} question={activeTask.question} />}</div></motion.div>}</AnimatePresence>
                </motion.li>
              })}
            </AnimatePresence>
          </ol>}
